...
$(document).ready(function() {
  ...
  if(isBenesse){
    ...
    if (APP.CT) {
      ...
      if (errorCode) {
        debugtext('[Dﾁｬ]ERRORコード：' + errorCode);
      ...
      // 当日学習回数
      debugtext('[学習状況取得API]結果');
      debugtext(JSON.stringify(resultDetail));
...
var KVS = {
        ...
        if ("00" == status) {
          ...
          body.errors.forEach(function(elm) {
            if (null !== elm.error_code) {
              debugtext('[KVSpost:fail]'+elm.resource+':'+elm.error_code);
          ...
          body.errors.forEach(function(elm) {
            if (null !== elm.error_code) {
              if (elm.error_code == 'WE07-0111-R024' || elm.error_code == 'WE07-0112-R024') { // KVS未登録
                debugtext('ERROR:' + elm.resource + ':' + elm.error_code + ' -> KVS未登録');
                KVS.post(_afterFunc); // ■KVS
              } else {
                debugtext('[KVSget:fail]'+elm.resource+':'+elm.error_code);
    ...
    kvslib.put(sendJson, function(status, body, response) {
      try {
        if ("00" == status) {
          ...
          body.errors.forEach(function(elm) {
            if (null !== elm.error_code) {
              debugtext('[KVSput:fail]'+elm.resource+':'+elm.error_code);
  ...
  delete: function () {
    ...
    kvslib.del(sendJson, function(status, body, response) {
      try {
        if ("00" == status) {
          debugtext('[KVSdelete:done]');
          window.location.reload();
        } else {
          body.errors.forEach(function (elm) {
            if (null !== elm.error_code) {
              debugtext('[KVSdelete:fail]'+elm.resource+':'+elm.error_code);
...
function afterKVS(_afterFunc) {
  debugtext('[afterKVS]');
  debugtext(JSON.stringify(SAVEDATA));
...
/* KVS関連追加ここまで */
////////////////////////////////////////
function debugtext(_text) {
  if ($('.debug').length) $('.debug-area').append(_text + '<br>');
}
debugtext('[hostname] ' + _hostname);
$(".debug-open .debug-btn").on('click',function(){
  $('.debug').addClass('show');
})
$('.debug-close .debug-btn').on('click', function (e) {
  $('.debug').removeClass('show');
});
$('.debug .debug-btn-reload').on('click', function (e) {
  window.location.reload();
});

$(".kvsDel").on("click", function() {
  if (isBenesse) {
    KVS.delete(); // ■KVS
  }else{
    alert("KVS使用環境外")
...
// 値を設定するボタン押下
$(".dayChange").on("click", function() {
  SAVEDATA.clear_cnt = $('#pul_1').val();

  APP.comp_cnt = $('#pul_2').val();

  SAVEDATA.first_flg = $('#singleCheckbox').is(':checked') ? 1 : 0;

  $("[class^=star_]").removeClass("is_act");
  $("[class^=challenge]").removeClass("is_play");
  $("[class^=challenge]").removeClass("is_clear");
  $(".challenge1").addClass("is_play");
  for (var i = 2; i <= cnt; i++) {
    $("#page-top .challenge" + i + " img").attr('src', dir_url + 'img/btn/challenge' + i + '_b.png');
  }
  $('.debug').removeClass('show');
...
$(document).ready(function() {
  ...
  while (cnt2 >= 0) {
    ...
    $('#pul_2').append($('<option>', {
      value: cnt2,  // オプションの値
      text: cnt2    // オプションに表示されるテキスト
    }));
    cnt2-=1
  }  
});

$(".modal-close-btns").on("click",function(){
  $('#error').hide();
})
var SAVEDATA = {
    "clear_cnt":0,
...
function hiden_check(){
    ...
    // 秘伝のデータをJSONファイルから作成
    $.getJSON("json/qa.json", function(data) {
        // siren_1のデータを取得
...
function ok_putKvs(){
    debugtext("getしたSAVEDATA.clear_cnt："+SAVEDATA.clear_cnt)
